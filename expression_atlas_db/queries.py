...
def unpack_fields(fields: Dict, keep:Union[List[str],Callable,None]=None) -> pd.Series:
    """ 
    """
    if callable(keep):
...
def fetch_contrasts(session: base._Session, studies: Union[List[str],None]=None) -> pd.DataFrame:
    """ 
    """
    query = select(base.Contrast, base.Study) \
...
def fetch_samplecontrasts(
    ...
    query = select(base.Contrast, base.Study.velia_id, base.SampleContrast.contrast_side, base.Sample) \
...
def fetch_samples(
    ...
    """ 
    """
    query = select(base.Sample, base.Study.velia_id) \
                    .join(base.Study, base.Study.id == base.Sample.study_id)
    if studies:
        query = query.filter(base.Study.velia_id.in_(studies))

    samples = pd.read_sql(query, session.bind)
    samples = pd.concat([samples, samples['fields'].apply(lambda x: unpack_fields(x, keep_fields))], axis=1)
...
def query_differentialexpression(
    ...
    """ 
    """
    studies_query = select(base.Contrast, base.Study.velia_id) \
                        .join(base.Study, base.Contrast.study_id == base.Study.id) \
    ...
    if sequenceregions:
        transcript_query = transcript_query.filter(base.Transcript.transcript_id.in_(sequenceregions))
    ...
    sequenceregions_df = pd.concat([transcript_sequenceregions_df, gene_sequenceregions_df], axis=0)
    sequenceregions_df.set_index('id', inplace=True)

    differentialexpression_query = select(base.DifferentialExpression) \
                                            .filter(base.DifferentialExpression.sequenceregion_id.in_(sequenceregions_df.index)  &
                                            base.DifferentialExpression.contrast_id.in_(studies_df.index))

    differentialexpression_df = pd.read_sql(differentialexpression_query, session_redshift.bind)
...
def query_samplemeasurement(
    ...
    if contrasts:
        contrast_subquery = select(base.Study.velia_id, base.Contrast.contrast_name, 
                                base.SampleContrast.contrast_side, base.SampleContrast.sample_id) \
                                .filter(base.Study.velia_id.in_(studies)) \
        ...
        samples_query = select(contrast_subquery, samples_subquery) \
                                .join(contrast_subquery, samples_subquery.c.id == contrast_subquery.c.sample_id)
    ...
    samplemeasurement_query = select(base.SampleMeasurement) \
                                            .filter(base.SampleMeasurement.sequenceregion_id.in_(sequenceregions_df.index)  &
    ...
    samplemeasurement_df = samplemeasurement_df.merge(
                                                    samples_df[['velia_id','srx_id','atlas_group','fields']+
...
def build_contrast_metatable(
    ...
    """ 
    """
    samplecontrast_df = fetch_samplecontrasts(session, studies=studies, contrasts=contrasts).fillna('')
    ...
    samplecontrast_df.insert(
                        ...
                        samplecontrast_df.groupby(
                            ...
                            )['srx_id'].transform(len).astype(str),
                        )
    samplecontrast_df.drop('srx_id', inplace=True, axis=1)
    samplecontrast_df = samplecontrast_df.groupby(
                                    ['velia_id', 'contrast_name', 'contrast_side'],
                                    ).agg(lambda x: ','.join(x.unique()))
    for c in samplecontrast_df.columns[samplecontrast_df.columns.str.startswith('sample')]:
        samplecontrast_df[f'left_{c}'] = samplecontrast_df.loc[
                        samplecontrast_df.index.get_level_values('contrast_side') == 'left'][c]
        samplecontrast_df[f'right_{c}'] = samplecontrast_df.loc[
                        samplecontrast_df.index.get_level_values('contrast_side') == 'right'][c]

    samplecontrast_df.reset_index(inplace=True, drop=False)
    samplecontrast_df.drop(
        samplecontrast_df.columns[samplecontrast_df.columns.str.startswith('sample')].tolist()+['contrast_side'],
        inplace=True,
        axis=1,
        )
    samplecontrast_df.index = samplecontrast_df.apply(lambda x: f"{x['velia_id']} -- {x['contrast_name']}", axis=1)
    samplecontrast_df = samplecontrast_df.groupby(
                                [samplecontrast_df.index, 'velia_id', 'contrast_name']
                                ).agg(sum).reset_index(['velia_id','contrast_name'], drop=False)
